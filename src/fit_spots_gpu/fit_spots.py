...
class MaxFilterLayer(nn.Module):
    def __init__(self, delta=1)->None:
        ...
        if padding_check.is_integer():
            ...
            # logic for pad adjust may not be accurate
...
class BoxFilterBySlice(nn.Module):
    """
    slicewise box filter for 3d image
    asumes only a single channel
    which allows me to use 2D convolution
    to treat each z scice as a independant channel
    """
...
filter_max: nn.Module
    ...
    def fit_time(self, imS):
        ...
        print('cpu labeling time :', time()-start) 
        
        spot_image = cp.asarray(spot_image)
        start = time()
        Xh = density_label_gpu(spot_image)

        print('gpu labeling time:', time() - start)
    ...
    def test(self, n_spots=100, n_images=1):
        for i in range(n_images):
            ...
            Xh = self.fit_time(im)
            print(f"Time taken for {i}th image :",time()-start)
        return Xh, im
